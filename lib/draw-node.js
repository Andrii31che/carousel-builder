...
const SLIDE_FORMATS = {
  feed:   { w: 1080, h: 1350, label: 'IG / FB лента (4:5)' },
  square: { w: 1080, h: 1080, label: 'Квадрат (1:1)' },
  story:  { w: 1080, h: 1920, label: 'TikTok / Stories (9:16)' }
};
...
async function drawSlide(slide, opts, index, total) {
  const fmt = SLIDE_FORMATS[slide.format || opts.format || 'feed'] || SLIDE_FORMATS.feed;
  ...
  // Per-slide accent override (allows multi-accent carousels)
  const slideAccent = slide.accent || opts.accent;
  const accentEff = hasImage ? slideAccent : effectiveAccent(slideAccent, theme.bg);
  const ctaTextC = contrastText(slideAccent);
  ...
  if (isStat && slide.stat && slide.stat.value) {
    ...
    // Auto-shrink font if the value doesn't fit horizontally
    let vSize = SZ.statValue;
    ctx.font = `${fonts.weights.display} ${vSize}px ${fonts.display}`;
    while (vSize > 80 && ctx.measureText(vText).width > CW) {
      vSize -= 12;
      ctx.font = `${fonts.weights.display} ${vSize}px ${fonts.display}`;
    }
