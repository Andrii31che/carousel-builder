...
function drawWrappedAccent(ctx, lines, x, y, lineH, normalColor, accentColor) {
  let cy = y;
  for (const line of lines) {
    let cx = x;
    for (const seg of line) {
      ctx.fillStyle = seg.accent ? accentColor : normalColor;
...
async function drawSlide(slide, opts, index, total) {
  ...
  if (slide.eyebrow) {
    ...
    const ebFont = `700 ${SZ.eyebrow}px ${fonts.body}`;
    units.push({
      group: 'text',
  ...
  if (slide.headline) {
    ...
    const hFont = `${fonts.weights.display} ${hSize}px ${fonts.display}`;
    ctx.font = hFont;
    ...
    units.push({
      ...
      draw(y) {
        ctx.font = hFont;
        drawWrappedAccent(ctx, hLines, PAD, y, lh2, textColor, accentEff);
      }
    });
  }

  if (slide.body && !isStat) {
    const bFont = `${fonts.weights.body} ${SZ.body}px ${fonts.body}`;
    ctx.font = bFont;
    ...
    units.push({
      ...
      draw(y) {
        ctx.font = bFont;
        drawWrappedAccent(ctx, bLines, PAD, y, lh3, bodyColor, accentEff);
      }
    });
  }
  ...
  if (isStat && slide.body) {
    ...
    units.push({
      ...
      draw(y) {
        ctx.font = sFont;
        drawWrappedAccent(ctx, sLines, PAD, y, lh4, mutedColor, accentEff);
      }
    });
  }
